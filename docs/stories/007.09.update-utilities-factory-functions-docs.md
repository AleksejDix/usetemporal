...
9. All import paths are correct (minuta)

**Quality:**
10. Markdown passes linting (markdownlint)
11. Documentation builds without errors
12. No occurrences of: `toPeriod`, `defineUnit`, `temporal.units`, `minuta`
...
- [ ] **Grep for old API patterns**
  ...
  grep -r "minuta" utilities/ factory-functions/
...
- [ ] **Show correct usage patterns**
  ...
  import { isToday } from 'minuta/operations'
  ...
  const now = minuta.period(new Date(), 'day')
  const somePeriod = minuta.period(someDate, 'day')

  if (isToday(minuta.adapter, now, somePeriod)) {
    // somePeriod is today
  }
  ```
...
- [ ] **Fix package names**
  - Replace `minuta` with `minuta`
  - Replace `/adapter-*` with `minuta/*`

- [ ] **Update signature documentation**
  ```typescript
  import { createTemporal } from 'minuta'
  import type { CreateMinutaOptions, Temporal } from 'minuta'

  function createTemporal(options: CreateMinutaOptions): Temporal
...
- [ ] **Check actual implementations**
  ...
  grep -A 5 "export.*isToday" packages/minuta/src/

  # Check isWeekday signature
  grep -A 5 "export.*isWeekday" packages/minuta/src/

  # Check isWeekend signature
  grep -A 5 "export.*isWeekend" packages/minuta/src/

  # Check createTemporal signature
  grep -A 10 "export.*createTemporal" packages/minuta/src/
  ```
...
- [ ] **Check all import statements**
  - Should be `minuta`
  - Should be `minuta/operations`
  - Should be `minuta/native` (or other adapters)
...
- Add: `period(adapter, date, unit)` or `minuta.period(date, unit)`
- Verify actual signature from codebase

**create-temporal.md:**
- Replace: `minuta` → `minuta`
- Replace: `minuta/native` → `minuta/native`
...
```typescript
createTemporal(options: CreateMinutaOptions): Temporal
...
### Key References
- Actual code: `packages/minuta/src/`
...
2. **Replaced with `period()`** from `minuta/operations`
3. **Updated all import paths** to `minuta`
...
7. **Corrected all adapter imports** to use proper paths like `minuta/native`

**Verification Results:**
- ✓ Zero occurrences of `toPeriod` in utilities/ and factory-functions/
- ✓ Zero occurrences of `minuta` in utilities/ and factory-functions/
