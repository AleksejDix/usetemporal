...
6. Level 1 (Pure): ≤7KB gzipped
7. Level 2 (Builder): ≤12KB gzipped
8. Level 3 (Composables): ≤20KB gzipped
...
├── level-1-minimal.ts      # period + divide + native
├── level-1-full.ts         # all operations + native
├── level-2-basic.ts        # builder with few methods
├── level-2-full.ts         # builder with many methods
├── level-3-composables.ts  # full DX with reactivity
└── calendar.ts             # calendar utilities
```

- [ ] **Create level-1-minimal.ts** (AC#2)
```typescript
// Minimal Level 1 usage - should be ≤7KB
...
```typescript
// Basic Level 2 usage - should be ≤10KB
import { createTemporal } from 'minuta';
import { createNativeAdapter } from 'minuta/native';

export const minuta = createTemporal({
  adapter: createNativeAdapter(),
  weekStartsOn: 1
});

export const month = minuta.period(new Date(), 'month');
export const weeks = minuta.divide(month, 'week');
```

- [ ] **Create level-3-composables.ts**
```typescript
// Full Level 3 usage - should be ≤20KB
...
jobs:
  size-check:
    ...
    steps:
      ...
      - name: Build package
        run: npm run build --workspace=minuta

      - name: Check bundle sizes
        run: npm run size --workspace=minuta

      - name: Generate size report
        if: github.event_name == 'pull_request'
        run: |
          npm run size --workspace=minuta --json > size-report.json
  ...
  visualize:
    ...
    steps:
      ...
      - name: Upload visualization
        ...
        with:
          name: bundle-visualization
          path: packages/minuta/dist/stats.html
          retention-days: 30
        ...
        with:
          script: |
            ...
            github.rest.issues.createComment({
              ...
              body: `📊 [View bundle visualization](${artifactUrl})`
            });
```
...
[![Bundle Size - Level 1](https://img.shields.io/badge/Level%201-7KB-success)](https://github.com/AleksejDix/minuta)
[![Bundle Size - Level 2](https://img.shields.io/badge/Level%202-12KB-success)](https://github.com/AleksejDix/minuta)
[![Bundle Size - Level 3](https://img.shields.io/badge/Level%203-20KB-yellow)](https://github.com/AleksejDix/minuta)
...
We enforce strict bundle size budgets to maintain Minuta's performance promise.

### Budget Definitions

| Scenario | Budget | Rationale |
|----------|--------|-----------|
| Level 1 - Minimal | ≤7KB | period + divide + native adapter |
| Level 1 - Full Ops | ≤10KB | All operations + native adapter |
| Level 2 - Basic | ≤12KB | Builder with few methods |
| Level 2 - Full | ≤15KB | Builder with many methods |
| Level 3 - Composables | ≤20KB | Full DX with reactivity |
| Calendar | ≤10KB | Calendar utilities standalone |
...
async function analyzeTreeShaking() {
  ...
  console.log(`📦 Full Package: ${distSize} bytes (${fullGzip} gzipped)`);
  console.log('');
    ...
    steps:
      ...
      - name: Commit size report
        run: |
          ...
          git config user.email "botminuta.com"
