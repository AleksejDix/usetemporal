...
These files are already "extracted" from the active codebase and ready to be copied to the Vue package in Story 011.03. Creating an intermediate `minuta-vue-extracted` directory is no longer necessary.

**See:** `STORY-011-SERIES-STATUS.md` for full explanation and validation findings.

**Next Step:** Proceed directly to Story 011.03 (corrected version) to create `minuta-vue` package from `.removed` files.
...
**so that** they can be easily moved to the new `minuta-vue` package.

## Acceptance Criteria

1. Create new directory `packages/minuta-vue-extracted/` for Vue-specific code
...
- [ ] Create extraction directory structure (AC: 1, 6)
  - [ ] Create `packages/minuta-vue-extracted/` directory
  - [ ] Create `packages/minuta-vue-extracted/src/` directory
  - [ ] Create `packages/minuta-vue-extracted/package.json` with @vue/reactivity dependency
  - [ ] Create `packages/minuta-vue-extracted/tsconfig.json` extending from shared config
  - [ ] Add to workspace in root `package.json`

- [ ] Move createTemporal implementation (AC: 2)
  - [ ] Copy `src/createTemporal.ts` to `minuta-vue-extracted/src/Minuta.ts`
  - [ ] Rename function from `createTemporal` to `Minuta`
...
- [ ] Move usePeriod composable (AC: 3)
  - [ ] Copy `src/composables/usePeriod.ts` to `minuta-vue-extracted/src/usePeriod.ts`
...
- [ ] Configure extracted package (AC: 5, 6, 7)
  ...
  - [ ] Add minuta to dependencies (local workspace reference)
  - [ ] Configure build script
  - [ ] Test compilation: `npm run build --workspace=minuta-vue-extracted`

- [ ] Update tests (AC: 8)
  - [ ] Create test directory in extracted package
  - [ ] Move relevant tests from core to extracted package
  - [ ] Update test imports
  - [ ] Run tests: `TZ=UTC npm test --workspace=minuta-vue-extracted`

- [ ] Document extraction (AC: 9)
  - [ ] Create README.md in extracted directory
  - [ ] Document purpose: "Temporary extraction for migration to minuta-vue"
...
This story is a **preparatory extraction step** before creating the official `minuta-vue` package. By extracting Vue-specific code to a separate directory first, we:
...
```
packages/minuta-vue-extracted/
├── src/
│   ├── Minuta.ts             ← Renamed from createTemporal
...
**Renaming createTemporal → Minuta:**
...
- `minuta`: workspace:* (for core operations)

**Temporary Nature:**
This is a **temporary extraction directory**. In story 011.03, this will become the official `minuta-vue` package with proper naming, versioning, and publication setup.

### Testing

**Test Location:** Create `packages/minuta-vue-extracted/src/` for colocated tests

**Test Standards:**
- Use Vitest
- Test both `Minuta` and `usePeriod` reactivity
...
```bash
export TZ=UTC
npm test --workspace=minuta-vue-extracted
```
