...
## Story
**As a** maintainer of the Minuta monorepo,  
...
console.warn('minuta/native is deprecated. Please use minuta/native instead.');
export * from 'minuta/native';
```
...
console.warn(
  "[/adapter-xxx] DEPRECATED: This package has been moved to minuta/xxx. " +
  "Please update your imports..."
);
export * from "minuta/xxx";
```
...
1. **Legacy Packages Removal**: ❌ INCORRECT CLAIM
   ...
   - However: Found residual adapter test directories in `/packages/minuta/packages/`
   - Meta package still has dependency on `minuta/native` in package.json
...
- [ ] Remove residual adapter test directories in minuta package
- [ ] Update minuta package.json to remove adapter dependency
...
**Note**: While the core implementation is good, the Dev Agent Record contains inaccurate claims about complete removal. The packages were partially removed but cleanup is incomplete. The minuta meta package still references the deprecated adapter package which will cause issues.