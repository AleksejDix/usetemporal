...
## Description
Create comprehensive documentation for the UNITS constant API, explaining its purpose, usage patterns, and integration with the broader Minuta library following the "Calculus for Time" philosophy.
...
The `UNITS` constant provides a read-only array of all supported time units in Minuta.
