...
**I want** to consolidate the core, minuta, and calendar-units packages into a single package,  
**so that** users have a simpler installation process and we reduce maintenance overhead.

## Acceptance Criteria
1. All functionality from packages/core, packages/minuta, and packages/calendar-units is consolidated into packages/minuta
...
- [x] Analyze current package structure and dependencies (AC: 1, 6)
  ...
  - [x] Document all exports from packages/minuta
...
- [x] Create consolidated package structure (AC: 1, 7)
  - [x] Set up new directory structure in packages/minuta
...
- [x] Migrate and consolidate tests (AC: 3)
  - [x] Move tests from packages/core to packages/minuta
  - [x] Move tests from packages/calendar-units to packages/minuta
...
│   │   └── index.ts
│   └── package.json
├── minuta/            # Meta package that re-exports core
│   ├── src/
│   │   └── index.ts
│   └── package.json
...
```
packages/
└── minuta/            # Single consolidated package
    ├── src/
...
1. The current minuta package appears to be a meta-package that just re-exports core
...
- Documented exports from core/index.ts, minuta/index.ts, calendar-units/index.ts
- Core has all main functionality, minuta is meta package, calendar-units adds stableMonth
- Identified @vue/reactivity as only runtime dependency
- New package will be minuta based on README naming pattern
- Found that packages/minuta already contained a full copy of core functionality
- Calendar units were already present in packages/minuta/src/calendar

### Completion Notes List
1. Discovered that packages/minuta already contained all core functionality, making this largely a cleanup task
2. Removed packages/core and packages/calendar-units directories  
3. Updated package.json for minuta to include all dependencies and proper exports
4. Fixed imports in example Vue project to use minuta
...
### File List
- Modified: /packages/minuta/package.json
- Modified: /packages/minuta/src/index.ts
- Modified: /packages/minuta/src/calendar/stableMonth.ts
- Modified: /packages/minuta/src/calendar/__tests__/integration.test.ts
- Modified: /packages/minuta/src/calendar/__tests__/stableMonth.test.ts
- Modified: /examples/vue/package.json
- Modified: /examples/vue/src/composables/Minuta.ts
