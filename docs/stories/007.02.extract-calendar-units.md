...
6. Add `minuta/calendar` export to package.json
...
export function createStableMonth(minuta: any, date: Date): any {
  const { adapter, weekStartsOn } = minuta;
  // ...
}
```
...
**Before:**
```typescript
const minuta = createTemporal({ adapter, weekStartsOn: 1, date: new Date() });
...
- [ ] Find calendar usage in packages/minuta-vue/src/components/CalendarExample.vue
- [ ] Update imports to use `minuta/calendar`
...
```bash
echo 'import { period } from "minuta/operations"; export { period }' > test-no-calendar.js
npm run build
...
```bash
echo 'import { createStableMonth } from "minuta/calendar"; export { createStableMonth }' > test-calendar.js
...
- [ ] Verify export types work correctly
  - Test importing types: `import type { StableMonth } from 'minuta/calendar'`
...
Non-calendar users save 5-10KB by not importing `minuta/calendar`
...
**Examples:**
- `packages/minuta-vue/src/components/CalendarExample.vue` - Update imports
...
- Calendar users: Import explicitly from 'minuta/calendar'
...
✅ **Clean Separation:** Period.ts no longer imports calendar functions. Comment explains users should import from 'minuta/calendar' directly
