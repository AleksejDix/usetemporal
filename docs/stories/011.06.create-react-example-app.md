...
**As a** React developer evaluating Minuta,
**I want** a working React example application,
**so that** I can see how to integrate Minuta-react into my projects.

## Acceptance Criteria

1. Create `packages/minuta-react/examples/` directory
2. Initialize React application with Vite
3. Install minuta-react dependency
4. Implement calendar component using Minuta and usePeriod
...
- [ ] Initialize React app (AC: 1, 2)
  - [ ] Create `packages/minuta-react/examples/` directory
  - [ ] Run `npm create vite@latest . -- --template react-ts`
  - [ ] Update package.json name to `@minuta-examples/react` *(legacy — replaced by shared package-level Vite config)*
  - [ ] Add to root workspace if needed

- [ ] Install dependencies (AC: 3)
  - [ ] Add minuta-react: workspace:*
  - [ ] Add any UI libraries needed (optional)
  - [ ] Run npm install

- [ ] Create Calendar component (AC: 4, 5, 6, 7)
  - [ ] Create src/components/Calendar.tsx
  - [ ] Implement Minuta hook
...
import { useMinuta, usePeriod, divide, createNativeAdapter } from 'minuta-react'
...
export function Calendar() {
  ...
  return (
    <div>
      ...
      <button onClick={minuta.previous}>Previous</button>
      <button onClick={minuta.next}>Next</button>
...
// PATTERN: React state management with Minuta
...
> **Note**: The standalone `@minuta-examples/react` workspace described here has since been replaced by running the demo through the package-level Vite config, so this JSON is retained only for historical reference.

```json
{
  "name": "@minuta-examples/react",
  "version": "1.0.0",
  "private": true,
  "type": "module",
  "scripts": {
